...
═══════════════════════════════════════════════════════════════════════════
         ...
         BRANDS API ROUTER - HANDLES LISTING BRANDS AND RETRIEVING
                BRAND DETAILS INCLUDING LOGOS AND IMAGES
...
async def list_brands():
    try:
        if not DATA_DIR.exists():
            raise HTTPException(status_code=404, detail="Brands directory not found")
                ...
                if brand_json_path.exists():
                    try:
                        with open(brand_json_path, 'r') as f:
                            ...
                            brand_name = brand_data.get('brand_name', brand_dir.name)
                    except Exception:
                        pass
                
                brands.append({'brand_id': brand_dir.name, 'brand_name': brand_name})
...
async def get_brand_details(brand_id: str):
    try:
        ...
        if not brand_json_path.exists():
            raise HTTPException(status_code=404, detail=f"brand.json not found for brand '{brand_id}'")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get brand details: {str(e)}")
